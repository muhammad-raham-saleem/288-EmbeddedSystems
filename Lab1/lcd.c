...
{
    const char banner[] = "                    Microcontrollers are fun!   ";
    ...
    {
        lcd_clear();
        lcd_setCursorPos(0, 0);   // row 0, col 0 set to start of screen

        // Print 20-character window which is the lcd width
        int i=0;
        for(; i < LCD_WIDTH; i++)
        {
            char c = banner[(offset + i) % bannerLen];
            lcd_putc(c);
        }

        offset = (offset + 1) % bannerLen;  // move left by one

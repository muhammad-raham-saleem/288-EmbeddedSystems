...
while(1){
    if(uart_available()) {
        char data = uart_receive_nonblocking();
        if(data == 's') {
            break;
        }
...
//            }//end of  new while
        ...
        if(objCounter > 0) {
            ...
            if(smallobjectangle < 90){ // object is to the right
                float turnAngle = 90 - (smallobjectangle*1.25);  //1.25 added for calibration Calculate how much right of center
...
//the following is what we used in lab 3
//int main(void){
//    // initialization
//    timer_init();
//    lcd_init();
